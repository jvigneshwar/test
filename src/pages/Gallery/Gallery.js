    ...
    <div className='gallery-outer'>
      ...
      {loading ?
        <div className='gallery-loading'>
          Loading
          <img src={require("../../assets/load.gif")} className='load' alt="loadig" />
        </div>
        :
        <>
          <div className={`gallery-image-bg ${activeImage!==null && 'gallery-image-bg-active'}`}></div>
          <div className='gallery'>
            {Object.keys(images).map((ele, ind) => {
              return (
                <div className={`gallery-image-container ${activeImage === ind && 'gallery-image-container-active'}`} key={ind} onClick={() => { setActiveImage(ind); setActiveImageArr(images[ele]) }}>
                  <img src={images[ele][0]} alt={ele.asset_id} className='gallery-image' />
                  <div className="gallery-content">{ele.split("=")[0]?.replaceAll("-", " ")}<br /><span className='gallery-content-inner'>{ele.split("=")[1] ? ele.split("=")[1].replaceAll("!", "/") : '00/00/0000'}</span></div>
                </div>
              )
            }
            )}
      ...
      {activeImage !== null &&
        <>
          ...
          {folderIndex !== activeImageArr.length - 1 &&
